...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if processing_class is None:
            if "Qwen2-VL" in model_id or "Qwen2.5-VL" in model_id or "VideoRFT" in model_id or "Aria" in model_id or True:
                processing_class = AutoProcessor.from_pretrained(model_id)
                pad_token_id = processing_class.tokenizer.pad_token_id
                processing_class.pad_token_id = pad_token_id
                processing_class.eos_token_id = processing_class.tokenizer.eos_token_id
                if "Qwen" in model_id or "Qwen2.5-VL" in model_id or "VideoRFT" in model_id:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            prompt_completion_ids = unwrapped_model.generate(**prompt_inputs, use_model_defaults=False, generation_config=self.generation_config)
        ...
        x_clamped = torch.clamp(ref_per_token_logps - per_token_logps, min=-10, max=10)  # limit the range of x to [-10, 10]
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            # If semantic reward is used, set the reward to 0 when accuracy is 0
            if reward_func.__name__ == "semantic_reward":
                accuracy_rewards = rewards_per_func[:, 0]  # Assuming accuracy is the first reward function
                zero_accuracy_mask = (accuracy_rewards <= 0.0)
                rewards_per_func[zero_accuracy_mask, i] = 0.0
