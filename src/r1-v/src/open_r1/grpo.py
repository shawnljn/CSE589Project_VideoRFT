...
_SIGLIP_NAME = "siglip-so400m-patch14-384"   # SigLIP ckpt path
...
def semantic_reward(completions, data_type, path, w_scale=2.0, **kwargs):
    ...
    if data_type[0] == "image":
        image_path = './VideoRFT' + path[0][1:]
        image = Image.open(image_path).convert("RGB")
        inputs = _siglip_processor(images=[image] * len(think_contents),
                                   text=think_contents,
                                   return_tensors="pt",
                                   truncation=True,
                                   padding="max_length").to(_siglip_model.device)
        with torch.no_grad():
